...
const BlochSphere: React.FC = () => {
  ...
  const updateBlochVector = () => {
    ...
    // ensure geometry exists and use setFromPoints
    try {
      (blochVectorRef.current.geometry as Three.BufferGeometry).setFromPoints(
        points
      );
    } catch (e) {
      // fallback: replace geometry
      blochVectorRef.current.geometry =
  ...
  const canvasStyle: React.CSSProperties = {
    ...
    height: "100%",
    cursor: "grab",
    zIndex: 0, // canvas stays behind any sibling UI
  };
  ...
  useEffect(() => {
    ...
    // Make scene background null so renderer clear color alpha controls transparency
    ...
    // Make canvas transparent and style it to stay within container and behind UI
    ...
    canvasEl.style.zIndex = "0"; // behind sibling UI
    // Keep pointer events enabled so OrbitControls works (set to 'none' if you want clicks to pass through)
    ...
    const animate = () => {
      ...
      if (blochVectorRef.current) {
        const pts = (blochVectorRef.current.geometry as Three.BufferGeometry)
          .attributes.position;
        if (pts && (pts as any).count >= 2) {
          const end = new Three.Vector3().fromBufferAttribute(pts as any, 1);
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={containerStyle}>
            {/* mountRef container is positioned; renderer.domElement is appended into it */}
        ...
        <div
          style={{
            ...
            zIndex: 2, // make sure notes sit above canvas
          }}
        >
          <h3
            style={{
